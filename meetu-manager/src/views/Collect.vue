...
import { delBatch, collectPage,  delOne, saveOrUpdate } from "@/api/collectApi";
...
// 新增对话框
const dialogAdd = () => {
  dialogData.title = '新增收藏'
  resetDialog({})
}
// 编辑用户信息 随便查看详情
const dialogEdit = (row) => {
  dialogData.title = '收藏详情'
  resetDialog(row)
}
// 重置 对话框
...
const importDataUrl = ref('/api/collect/import')
...
// 导出数据
const exportAll = () => {
 window.open("http://localhost:8848/api/collect/export")
}
...
const save = () => {
  dialogFormRef.value.validate(valid => {   // valid就是校验的结果
    if (valid) {
      saveOrUpdate('/collect', dialogData.formData).then(res => {
        if (res.code === '200') {
...
<template>
  <div class="home">
    ...
    <div class="main-search">
      <el-input v-model="name" placeholder="请输入收藏关键字"/>
      <el-button
          type="primary"
          @click="searchcollect"
      >
    ...
    <div style="margin: 10px 0;">
          <el-button
              v-show="data.pageMenus.includes('collect.add')"
              type="primary"
              style="color: white"
              color="#00bd16"
              @click="dialogAdd"
          ...
          <el-button
              v-show="data.pageMenus.includes('collect.export')"
              type="primary"
              style="color: white"
              @click="exportAll"
          >
          ...
          <el-upload
              v-show="data.pageMenus.includes('collect.import')"
              class="upload-box"
          ...
          <el-popconfirm title="您确定要执行此操作吗？" @confirm="confirmDelBatch">
            <template #reference>
              <el-button
                  v-show="data.pageMenus.includes('collect.deleteBatch')"
                  type="danger"
                  style="color: white"
              >
                <el-icon style="vertical-align: middle">
    ...
    <div style="margin: 20px 0">
      ...
      >
       <el-table-column type="selection" width="55"/>
        ...
        <el-table-column prop="dynamicName" label="收藏动态标题"></el-table-column>
         ...
         <template #default="scope">
           ...
           <el-popconfirm title="您确定要删除吗？" @confirm="del(scope.row)">
             <template #reference>
               <el-button
                   type="danger"
                   v-show="data.pageMenus.includes('collect.delete')"
               >
                 删除
               </el-button>
             </template>
    ...
    >
      ...
      >
        ...
        <el-form-item prop="dynamicId" label="动态内容">
        </el-form-item>
        <Editor
