...
import {delBatch, tagPage, delOne, saveOrUpdate} from "@/api/tagApi";
...
// 新增对话框
const dialogAdd = () => {
  dialogData.title = '新增话题'
  resetDialog({})
}
// 编辑用户信息 随便查看详情
const dialogEdit = (row) => {
  dialogData.title = '编辑话题'
  resetDialog(row)
}
// 重置 对话框
...
const importDataUrl = ref('/api/tag/import')
...
// 导出数据
const exportAll = () => {
 window.open("http://localhost:8848/api/tag/export")
}
...
const save = () => {
  dialogFormRef.value.validate(valid => {   // valid就是校验的结果
    if (valid) {
      saveOrUpdate('/tag', dialogData.formData).then(res => {
        if (res.code === '200') {
...
<template>
  <div class="home">
    ...
    <div class="main-search">
      <el-input v-model="name" placeholder="请输入话题关键字"/>
      <el-button
          type="primary"
          @click="searchtag"
      >
    ...
    <div style="margin: 10px 0;">
          <el-button
              v-show="data.pageMenus.includes('tag.add')"
              type="primary"
              style="color: white"
              color="#00bd16"
              @click="dialogAdd"
          ...
          <el-button
              v-show="data.pageMenus.includes('tag.export')"
              type="primary"
              style="color: white"
              @click="exportAll"
          >
          ...
          <el-upload
              v-show="data.pageMenus.includes('tag.import')"
              class="upload-box"
          ...
          <el-popconfirm title="您确定要执行此操作吗？" @confirm="confirmDelBatch">
            <template #reference>
              <el-button
                  v-show="data.pageMenus.includes('tag.deleteBatch')"
                  type="danger"
                  style="color: white"
              >
                <el-icon style="vertical-align: middle">
      ...
      >
       ...
       <el-table-column label="操作" width="180">
         <template #default="scope">
           <el-button
               v-show="data.pageMenus.includes('tag.edit')"
               type="primary"
           ...
           <el-popconfirm title="您确定要删除吗？" @confirm="del(scope.row)">
             <template #reference>
               <el-button
                   type="danger"
                   v-show="data.pageMenus.includes('tag.delete')"
               >
                 删除
               </el-button>
             </template>
    ...
    >
      ...
      >
        ...
        <el-form-item prop="comments" label="备注">
          <el-input v-model="dialogData.formData.comments" autocomplete="off"></el-input>
        </el-form-item>
