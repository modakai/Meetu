...
import { delBatch, praisePage,  delOne, saveOrUpdate } from "@/api/praiseApi";
...
const resetDialog = (data) => {
  ...
  nextTick(() => {
    ...
    valueHtml.value = dialogData.formData.dynamic.content
...
const importDataUrl = ref('/api/praise/import')
...
// 导出数据
const exportAll = () => {
 window.open("http://localhost:8848/api/praise/export")
}
...
const save = () => {
  dialogFormRef.value.validate(valid => {   // valid就是校验的结果
    if (valid) {
      saveOrUpdate('/praise', dialogData.formData).then(res => {
        if (res.code === '200') {
...
<template>
  <div class="home">
    ...
    <div class="main-search">
      <el-input v-model="name" placeholder="请输入点赞关键字"/>
      <el-button
          type="primary"
          @click="searchpraise"
      >
    ...
    <div style="margin: 10px 0;">
          <el-button
              v-show="data.pageMenus.includes('praise.add')"
              type="primary"
              style="color: white"
              color="#00bd16"
              @click="dialogAdd"
          ...
          <el-button
              v-show="data.pageMenus.includes('praise.export')"
              type="primary"
              style="color: white"
              @click="exportAll"
          >
          ...
          <el-upload
              v-show="data.pageMenus.includes('praise.import')"
              class="upload-box"
          ...
          <el-popconfirm title="您确定要执行此操作吗？" @confirm="confirmDelBatch">
            <template #reference>
              <el-button
                  v-show="data.pageMenus.includes('praise.deleteBatch')"
                  type="danger"
                  style="color: white"
              >
                <el-icon style="vertical-align: middle">
    ...
    <div style="margin: 20px 0">
      ...
      >
       <el-table-column type="selection" width="55"/>
        ...
        <el-table-column prop="user.name" label="点赞用户"></el-table-column>
        <el-table-column prop="dynamic.name" label="点赞动态标题"></el-table-column>
         ...
         <template #default="scope">
           ...
           <el-popconfirm title="您确定要删除吗？" @confirm="del(scope.row)">
             <template #reference>
               <el-button
                   type="danger"
                   v-show="data.pageMenus.includes('praise.delete')"
               >
                 删除
               </el-button>
             </template>
